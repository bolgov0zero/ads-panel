...
# Устанавливаем права и создаем директории для видео и базы данных
...
RUN echo "ServerName localhost" >> /etc/apache2/apache2.conf && \
    a2enmod rewrite ssl && \
    echo "Alias /videos /opt/ads" >> /etc/apache2/conf-available/videos.conf && \
    echo "<Directory /opt/ads>" >> /etc/apache2/conf-available/videos.conf && \
    echo "    Options Indexes FollowSymLinks" >> /etc/apache2/conf-available/videos.conf && \
    echo "    AllowOverride All" >> /etc/apache2/conf-available/videos.conf && \
    echo "    Require all granted" >> /etc/apache2/conf-available/videos.conf && \
    echo "</Directory>" >> /etc/apache2/conf-available/videos.conf && \
    a2enconf videos
